...
class SEARSAttacker(Attacker):
    def __init__(self, **kwargs):
        ...
        self.ps = self.paraphrase_scorer.ParaphraseScorer(gpu_id=1)
        ...
        right_preds = self.config["clsf"].get_pred(right_val)
        print(len(right_val))
        for i, sentence in enumerate(right_val):
            ...
            print(len(flips[sentence]))

        print("find rules")
        tr2 = self.replace_rules.TextToReplaceRules(self.config["processor"], right_val, [], min_freq=0.005, min_flip=0.00, ngram_size=10)
        self.frequent_rules = []
        rule_idx = {}
        self.rule_flips = {}
        ...
        self.really_frequent_rules = [i for i in range(len(self.rule_flips))]
    ...
    def __call__(self, clsf, x_orig, target=None):
        ...
        fs = self.find_flips(x_orig, clsf, topk=200, threshold=-15)
        ...
        #really_frequent_rules = [i for i in range(len(rule_flips))]
