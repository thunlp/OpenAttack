...
class GANAttacker(Attacker):
    def __init__(self, **kwargs):
        ...
        :Package Requirements:
            * torch
        :Data Requirements: :py:data:`.AttackAssist.GNAE` :py:data:`.AttackAssist.SGNAE`
        ...
        if self.config['sst'] is False:  # snli
            self.word2idx, self.autoencoder, self.inverter, self.gan_gen, self.gan_disc = DataManager.load("AttackAssist.GNAE")
            self.maxlen = 10
        else:
            self.word2idx, self.autoencoder, self.inverter, self.gan_gen, self.gan_disc = DataManager.load("AttackAssist.SGNAE")
            self.maxlen = 100
